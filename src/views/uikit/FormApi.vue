...
export default {
    ...
    methods: {
        ...
        searchCustomer() {
            this.loading = true;
            axios
                .get(`https://api.restful-api.dev/objects/${this.customerId}`) // URL de la API para buscar un solo telefono por ID
                .then((response) => {
                    if (response.data) {
                        this.customerData = {
                            id: response.data.id,
                        ...
                        this.customerData = null; // Si no se encuentra el telefono
                    }
                    this.loading = false;
                })
                .catch((error) => {
            ...
            axios
                ...
                .then((response) => {
                    if (Array.isArray(response.data)) {
                        ...
                        this.customers1 = [response.data]; // Un solo objeto
                    }
                    this.loading1 = false;
                })
                .catch((error) => {
...
<template>
    ...
    <div class="card mt-6">
        ...
        <DataTable :value="customers1" :paginator="true" :rows="10" dataKey="id" :rowHover="true" v-model:filters="filters1" filterDisplay="menu" :loading="loading1" :globalFilterFields="['name', 'color', 'capacity']" showGridlines>
            <template #header>
            ...
            <template #empty> No se encontraron teléfonos. </template>

            <template #loading> Cargando datos de los teléfonos. Por favor espere. </template>
